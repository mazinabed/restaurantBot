...
// require('dotenv').config();
// const TOKEN = process.env.TOKEN
// //"6456398897:AAEP2j09JLgqluudftxWMSxDujmFT4sws6g";
// const bot = new Telegraf(TOKEN);
...
//   webhook: {
//     // Public domain for webhook; e.g.: example.com
//     domain: "https://restaurant-bot.vercel.app/api",
...
const TOKEN = process.env.TOKEN;
const VERCEL_URL = "https://restaurant-bot.vercel.app"; // Add this line to get the Vercel deployment URL
const bot = new Telegraf(TOKEN);

// Set up the webhook with the Vercel deployment URL
const webhookPath = "/api"; // Customize the path
bot.telegram.setWebhook(`${VERCEL_URL}${webhookPath}`);
const web_link = "https://testingbot-8rcz.onrender.com";
// Register a route to handle incoming updates from Telegram
module.exports = async (req, res) => {
  try {
    // Use the bot instance to handle the incoming update
...
bot.on('message', (ctx) => {
  try {
    ...
    for (const item of cartItems) {
      ...
      // Send the order details to the Telegram channel
      bot.telegram.sendMessage(CHANNEL_ID, message, { parse_mode: 'Markdown' });
    }
  } catch (error) {
    console.error('Error parsing received data:', error);
  }
...
// Ensure the webhook path is registered
bot.telegram.setWebhook(`${VERCEL_URL}${webhookPath}`);
